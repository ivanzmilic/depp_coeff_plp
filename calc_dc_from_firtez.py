...
if (__name__ == '__main__'):
    ...
    if rank == 0:
        ...
        input_atmos = 'test_input.fits'


        #cdfs = [LazyRadynData(f) for f in grid_files_full_path]

        atmos = fits.open(input_atmos)[0].data
        print ("atmosphere loaded...")
        NPAR, NT, NX, NY, NZ = atmos.shape
        print (NPAR, NT, NX, NY, NZ)
        atmos = atmos.reshape(NPAR, NT*NX*NY, NZ)
        
        #overseer_work(cdfs, grid_files, output_dir)
        overseer_work(atmos, output_file)l
    else:
        worker_work(rank)
        pass
