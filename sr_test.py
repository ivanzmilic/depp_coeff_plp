...
from lightweaver.rh_atoms import H_6_atom, C_atom, O_atom, Si_atom, Al_atom, CaII_atom, Fe_atom, He_9_atom, MgII_atom, N_atom, Na_atom, S_atom, SrI_atom
...
def synth_8542(atmos, conserve, useNe, wave):
    ...
    # program will use.
    ctx = lw.Context(atmos, spect, eqPops, conserveCharge=conserve, Nthreads=1, formalSolver='piecewise_linear_1d')
    # Iterate the Context to convergence (using the iteration function now
    # provided by Lightweaver)
    lw.iterate_ctx_se(ctx, prd=False)
...
#wave = np.linspace(853.7, 854.7, 1001)
wave = np.linspace(460.4, 461.0, 601)
wave0 = lw.utils.air_to_vac(wave)

atmosRef = Falc82()
ctxRef, IwaveRef = synth_8542(atmosRef, conserve=False, useNe=True, wave=wave0)
#atmosCons = Falc82()
