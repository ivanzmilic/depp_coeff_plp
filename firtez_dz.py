...
class stk_profile3D(object):
  ...
  def write_profile(self, fname, vv=3, fmt_type=np.float32):
    ...
    def write_profile3D_v3(fname, data, fmt_type=np.float32):
      ...
      nnum_rec = np.int32(nnum_max_frec - 10.)
...
#    linestyle = ['-'] * len(profiles)
  ...
  if (len(pars)%2 != 0):
    ...
    ncols=sqrtabove
    nrows=np.int16(np.ceil(len(pars)/np.float64(ncols)))
  ...
  def dev_read_model3D_v3(ofile, nrec, dims, fmt_type, verbose=False):
    ...
    while (ipar<npar):
      ...
      if (ipar==0):
        ...
        model3D.set_vx(to_store.reshape(nx,ny,nz))
      elif (ipar==7):
        model3D.set_vy(to_store.reshape(nx,ny,nz))
      elif (ipar==8):
...
## Develpoing.
  ...
  def read_model3D_v3(ofile, nrec, dims, fmt_type, verbose=False):
    ...
    model3D = atm_model3D(nx, ny, nz)
    to_store = np.zeros(np.int64((npar*1.)*nx*ny*nz), dtype=np.float32)
...
class atm_model3D(object):
  ...
  def set_vx(self,array, axis=-1):
    self.set_attribute(array, 'vx', axis)
    return

  def set_vy(self,array, axis=-1):
    self.set_attribute(array, 'vy', axis)
    return
  ...
  def re_sample(self, new_nz, step=0.\
    ...
    for it_nx in range(self.nx):
      for it_ny in range(self.ny):
        ...
        f = interp1d(old_z, self.vx[it_nx, it_ny, :]\
            , kind=kind, fill_value=fill_value)
        narray[6, it_nx, it_ny, :] = f(new_z)
        f = interp1d(old_z, self.vy[it_nx, it_ny, :]\
            , kind=kind, fill_value=fill_value)
